...
<body>
	<?php
	$templateUrl = array(
		array('A-ha Shop | Minimal Elegant eCommerce HTML Template', 'http://deothemes.com/envato/aha/shop/index.html'),
		array('AllStore - MultiConcept eCommerce Shop Template', 'http://allstore-html.real-web.pro/'),
		array('BoShop - Multipurpose eCommerce HTML5 Template', 'http://demos.webicode.com/html/Bo-shop-html/'),
	...
	<script>
		$(".web-thumbnail").each(function(key, value){
			...
			$.get(linkUrl, function(data){
				...
				var content = $("<a>", {href: template[1], target: "_blank"});
				$("<img>", {class: "image img-responsive img-thumbnails", src: dataURI}).appendTo(content);
				$("<div>", {class: "caption", html: "<p>"+template[0]+"</p>"}).appendTo(content);
				$(value).html(content);
			});		
		});
	</script>
